...
public class ImageController {
    ...
    public String postRequestPremium(@RequestBody byte[] imgbyte, @RequestHeader(value = "Filter") String filter,  @RequestHeader(value = "addText", required=false) String addText,  @RequestHeader(value = "x", required=false) String x,  @RequestHeader(value = "y", required=false) String y) throws IOException, ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        if(addText == null){ //se non esiste l'altro parametro oltre il filtro (fltro AddText)
            ...
            classeFiltro = Class.forName("com.mycompany.serverspring.imageFilter." + filter).getConstructor(byte[].class, String.class, String.class, String.class);
            this.img = (BufferedImgBase) classeFiltro.newInstance(imgbyte, addText, x , y);
        }      
        //call applicaFiltro()
        this.newByte = this.img.applicaFiltro();
