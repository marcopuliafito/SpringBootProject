...
public class SendImage extends HttpsClient {
    ...
    //overload costruttore con proprietà per scrivere su immagine con coordinate
    public SendImage(byte[] img, String filter, String token,  String addText, int x, int y) {
        super();
        this.img = img;
        this.filter = filter;
        this.token = token;
    ...
    public String post_request_image() {
        try {
            ...
            if (this.addText != null){
                ...
                connection.setRequestProperty("x", this.x);
                connection.setRequestProperty("y", this.y);
            }
            if (this.token != null)
